...
CONTAINS
   ...
   if(.not.allocated(tauaer_sw)) allocate(tauaer_sw(1, 1, 1, 1))
   if(.not.allocated(ssaaer_sw)) allocate(ssaaer_sw(1, 1, 1, 1))
   if(.not.allocated(asyaer_sw)) allocate(asyaer_sw(1, 1, 1, 1))
...
wrf_err_message )
     ...
     IF ((sw_physics .NE. RRTMG_SWSCHEME) .AND. (sw_physics .NE. RRTMG_SWSCHEME_FAST) &
        ...
        DO j=jts,jte
           ...
           ENDDO
        ENDIF
        IF ( PRESENT( qs  ) .AND. ICLOUD.eq.3) THEN
           DO j=jts,jte
           DO k=kts,kte
           DO i=its,ite
