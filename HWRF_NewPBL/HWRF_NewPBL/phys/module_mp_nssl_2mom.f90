...
MODULE module_mp_nssl_2mom
  ...
  logical :: idonic = .false.
      ...
      integer, parameter :: nqiacralpha = 120 
      integer, parameter :: nqiacrratio =  50 
      ...
      double precision :: gamxinflu(0:nqiacrratio,0:nqiacralpha,8,2) 
    ...
    DATA mmgraupvt(:,3) / 0.67819, 0.63789, 0.62197, 0.61240, 0.60572, 0.60066, 0.59663, 0.59330, 0.59048 /
      ...
      real, parameter :: cwradx = 40.e-6, xcradmx = cwradx    
...
29998 continue
 ...
 1100 continue
     ...
     &                 cwmasn,cwmasx,cwradn,cnina,cimn,cimx,   &
     &                 itype1,itype2,temcg,0,alpha,0,axh,bxh,axhl,bxhl)
     ...
     &  , qrmxd(mgs))
       ...
       ELSEIF ( ipconc .ge. 3 ) THEN
         ...
         IF ( ibiggopt == 2 .and. imurain == 1 ) THEN
           ...
           IF ( dbigg < Min(dfrz,dhmn) .and. ( ibiggsnow == 1 .or. ibiggsnow == 3 ) ) THEN 
     ...
     &            ((1. + alpha(mgs,lr))*(2. + alpha(mgs,lr))*(3. + alpha(mgs,lr)))
         ...
         IF ( crfrz(mgs) .gt. qxmin(lh) ) THEN
           ...
           ELSE
            ...
            IF ( ibfr .le. 1 ) THEN
             crfrzf(mgs) = frach*Min(crfrz(mgs), dble(qrfrz(mgs)/(bfnu*1.0*vr1mm*1000.0)*rho0(mgs)) ) 
            ELSEIF ( ibfr .eq. 5 ) THEN
             crfrzf(mgs) = frach*Min(crfrz(mgs), dble(qrfrz(mgs)/(bfnu*vfrz*1000.0)*rho0(mgs)) )*rzxh(mgs)  
            ELSEIF ( ibfr .eq. 2 ) THEN
             crfrzf(mgs) = frach*Min(crfrz(mgs), dble(qrfrz(mgs)/(bfnu*vfrz*1000.0)*rho0(mgs)) ) 
            ELSEIF ( ibfr .eq. 6 ) THEN
             crfrzf(mgs) = frach*Max(crfrz(mgs), dble(qrfrz(mgs)/(bfnu*9.*xv(mgs,lr)*1000.0)*rho0(mgs)) ) 
            ELSE
     ...
     &   + fwet2(mgs)*(qhlaci(mgs) + qhlacs(mgs)) )
      ...
      IF ( ehs(mgs) .gt. 0.0 ) THEN
        ...
        qhacs(mgs) = qhacs0(mgs) 
        chacs(mgs) = chacs0(mgs) 
        ehs(mgs) = 1.0 
     ...
     &  /((dzfacp+dzfacm))
      ...
      ELSEIF ( ciint(mgs) .gt. (ciintmx - (cx(mgs,li)))) THEN
