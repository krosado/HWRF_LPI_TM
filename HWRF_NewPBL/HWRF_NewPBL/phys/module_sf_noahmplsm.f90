...
CONTAINS
  ...
  SUBROUTINE TRANSFER_MP_PARAMETERS(VEGTYPE,SOILTYPE,SLOPETYPE,SOILCOLOR)
    ...
    EBLFOREST = EBLFOREST_TABLE
     ...
     CH2OP =  CH2OP_TABLE(VEGTYPE)       
     DLEAF =  DLEAF_TABLE(VEGTYPE)       
     Z0MVT =  Z0MVT_TABLE(VEGTYPE)       
       HVT =    HVT_TABLE(VEGTYPE)       
       HVB =    HVB_TABLE(VEGTYPE)       
       DEN =    DEN_TABLE(VEGTYPE)       
        RC =     RC_TABLE(VEGTYPE)       
     MFSNO =  MFSNO_TABLE(VEGTYPE)       
      SAIM =   SAIM_TABLE(VEGTYPE,:)     
      LAIM =   LAIM_TABLE(VEGTYPE,:)     
       SLA =    SLA_TABLE(VEGTYPE)       
    DILEFC = DILEFC_TABLE(VEGTYPE)       
    DILEFW = DILEFW_TABLE(VEGTYPE)       
     FRAGR =  FRAGR_TABLE(VEGTYPE)       
    LTOVRC = LTOVRC_TABLE(VEGTYPE)       

     C3PSN =  C3PSN_TABLE(VEGTYPE)       
      KC25 =   KC25_TABLE(VEGTYPE)       
       AKC =    AKC_TABLE(VEGTYPE)       
      KO25 =   KO25_TABLE(VEGTYPE)       
       AKO =    AKO_TABLE(VEGTYPE)       
    VCMX25 = VCMX25_TABLE(VEGTYPE)       
     AVCMX =  AVCMX_TABLE(VEGTYPE)       
        BP =     BP_TABLE(VEGTYPE)       
        MP =     MP_TABLE(VEGTYPE)       
      QE25 =   QE25_TABLE(VEGTYPE)       
       AQE =    AQE_TABLE(VEGTYPE)       
     RMF25 =  RMF25_TABLE(VEGTYPE)       
     RMS25 =  RMS25_TABLE(VEGTYPE)       
     RMR25 =  RMR25_TABLE(VEGTYPE)       
       ARM =    ARM_TABLE(VEGTYPE)       
    FOLNMX = FOLNMX_TABLE(VEGTYPE)       
      TMIN =   TMIN_TABLE(VEGTYPE)       

        XL =     XL_TABLE(VEGTYPE)       
      RHOL =   RHOL_TABLE(VEGTYPE,:)     
      RHOS =   RHOS_TABLE(VEGTYPE,:)     
      TAUL =   TAUL_TABLE(VEGTYPE,:)     
      TAUS =   TAUS_TABLE(VEGTYPE,:)     

       MRP =    MRP_TABLE(VEGTYPE)       
     CWPVT =  CWPVT_TABLE(VEGTYPE)       

     WRRAT =  WRRAT_TABLE(VEGTYPE)       
    WDPOOL = WDPOOL_TABLE(VEGTYPE)       
     TDLEF =  TDLEF_TABLE(VEGTYPE)       

     NROOT =  NROOT_TABLE(VEGTYPE)       
       RGL =    RGL_TABLE(VEGTYPE)       
     RSMIN =     RS_TABLE(VEGTYPE)       
        HS =     HS_TABLE(VEGTYPE)       
      TOPT =   TOPT_TABLE(VEGTYPE)       
     RSMAX =  RSMAX_TABLE(VEGTYPE)       
...
contains
  ...
  USE NOAHMP_PARAMETERS, ONLY: NROOT, &          
                               ISBARREN, ISURBAN 
  ...
  gpp = 0.0
   ...
   CALL ATM (SFCPRS  ,SFCTMP   ,Q2      ,                            &
     ...
     CALL PHENOLOGY (VEGTYP , SNOWH  , TV     , LAT   , YEARLEN , JULIAN , & 
...
"Namelist parameter DVEG unknown") 
    ...
    CALL PRECIP_HEAT(ILOC   ,JLOC   ,VEGTYP ,DT     ,UU     ,VV     , & 
                     ELAI   ,ESAI   ,FVEG   ,IST    ,                 & 
                     BDFALL ,RAIN   ,SNOW   ,FP     ,                 & 
    ...
    CALL ENERGY (ICE    ,VEGTYP ,IST    ,NSNOW  ,NSOIL  , & 
   ...
   IF (DVEG == 2 .OR. DVEG == 5) THEN
    CALL CARBON (NSNOW  ,NSOIL  ,VEGTYP ,DT     ,ZSOIL  , & 
  ...
  USE NOAHMP_PARAMETERS, ONLY: RAIR, CPAIR, TFRZ                     
  ...
  SUBROUTINE PHENOLOGY (VEGTYP , SNOWH  , TV     , LAT   , YEARLEN , JULIAN , & 
                        LAI    , SAI    , TROOT  , ELAI    , ESAI   , IGS)




  USE NOAHMP_PARAMETERS, ONLY : LAIM, SAIM, HVT, HVB, TMIN, &           
                                ISBARREN, ISICE, ISURBAN, ISWATER       
  ...
  SUBROUTINE PRECIP_HEAT (ILOC   ,JLOC   ,VEGTYP ,DT     ,UU     ,VV     , & 
                          ELAI   ,ESAI   ,FVEG   ,IST    ,                 & 
                          BDFALL ,RAIN   ,SNOW   ,FP     ,                 & 
                          CANLIQ ,CANICE ,TV     ,SFCTMP ,TG     ,         & 
                          QINTR  ,QDRIPR ,QTHROR ,QINTS  ,QDRIPS ,QTHROS , & 
  ...
  USE NOAHMP_PARAMETERS, ONLY : CH2OP, CWAT, CICE, TFRZ           
...
"Water budget problem in NOAHMP LSM")
 ...
 END SUBROUTINE ERROR
  ...
  SUBROUTINE ENERGY (ICE    ,VEGTYP ,IST    ,NSNOW  ,NSOIL  , & 
                     ISNOW  ,DT     ,RHOAIR ,SFCPRS ,QAIR   , & 
  ...
  USE NOAHMP_PARAMETERS, ONLY: BEXP, PSISAT, SMCMAX, SMCREF, SMCWLT, &  
                               HVT, Z0MVT, CWPVT, NROOT, &              
			       EG, &                                    
			       Z0SNO, MFSNO, &                          
			       GRAV, SB, TFRZ, RW, HVAP, HSUB, CPAIR, & 
			       ISURBAN 
  ...
  CALL  RADIATION (VEGTYP  ,IST     ,ICE     ,NSOIL   , & 
...
"STOP in Noah-MP")
 ...
 CALL PHASECHANGE (NSNOW   ,NSOIL   ,ISNOW   ,DT      ,FACT    , & 
  ...
  USE NOAHMP_PARAMETERS, ONLY: CSOIL, SMCMAX, &                          
			       TFRZ, TKICE, TKWAT, CICE, CWAT, CPAIR, &  
			       ISURBAN                                   
  ...
  USE NOAHMP_PARAMETERS, ONLY: DENICE, DENH2O, CICE, CWAT        
  ...
  USE NOAHMP_PARAMETERS, ONLY: QUARTZ, SMCMAX, &                   
			       TKICE                               
  ...
  SUBROUTINE RADIATION (VEGTYP  ,IST     ,ICE     ,NSOIL   , & 
                        SNEQVO  ,SNEQV   ,DT      ,COSZ    ,SNOWH   , & 
                        TG      ,TV      ,FSNO    ,QSNOW   ,FWET    , & 
                        ELAI    ,ESAI    ,SMC     ,SOLAD   ,SOLAI   , & 
                        FVEG    ,ILOC    ,JLOC    ,                   & 
  ...
  LOGICAL VEG  
   ...
   CALL ALBEDO (VEGTYP ,IST    ,ICE    ,NSOIL  , & 
  ...
  SUBROUTINE ALBEDO (VEGTYP ,IST    ,ICE    ,NSOIL  , & 
                     DT     ,COSZ   ,FAGE   ,ELAI   ,ESAI   , & 
                     TG     ,TV     ,SNOWH  ,FSNO   ,FWET   , & 
                     SMC    ,SNEQVO ,SNEQV  ,QSNOW  ,FVEG   , & 
                     ILOC   ,JLOC   ,                         & 
  ...
  USE NOAHMP_PARAMETERS, ONLY: RHOL, RHOS, TAUL, TAUS             
  ...
  CALL GROUNDALB (NSOIL   ,NBAND   ,ICE     ,IST     , & 
...
100 CONTINUE
  ...
  USE NOAHMP_PARAMETERS, ONLY: SWEMX, &                         
			       TFRZ                             
  ...
  SUBROUTINE GROUNDALB (NSOIL   ,NBAND   ,ICE     ,IST     , & 
                        FSNO    ,SMC     ,ALBSND  ,ALBSNI  ,COSZ    , & 
                        TG      ,ILOC    ,JLOC    ,                   & 
                        ALBGRD  ,ALBGRI  )                              

  USE NOAHMP_PARAMETERS, ONLY: ALBSAT, ALBDRY, ALBLAK, &       
			       TFRZ                            
  ...
  USE NOAHMP_PARAMETERS, ONLY: RC, HVT, HVB, DEN, XL, &     
  ...
  USE NOAHMP_PARAMETERS, ONLY: HVT, &               
			       SB, VKC, TFRZ, CPAIR 
...
"CRITICAL PROBLEM IN MODULE_SF_NOAHMPLSM:VEGEFLUX" )
  ...
  USE NOAHMP_PARAMETERS, ONLY: SB, VKC, TFRZ, CPAIR, ISURBAN             
  ...
  USE NOAHMP_PARAMETERS, ONLY: GRAV, VKC, CPAIR            
...
"STOP in Noah-MP")
  ...
  USE NOAHMP_PARAMETERS, ONLY: CZIL, &            
			       GRAV               
  ...
  USE NOAHMP_PARAMETERS, ONLY: BP, MP, FOLNMX, QE25, KC25, KO25, &      
                               AKC, AKO, VCMX25, AVCMX, C3PSN, &        
			       TFRZ                                     
  ...
  USE NOAHMP_PARAMETERS, ONLY: RGL, RSMIN, RSMAX, HS, TOPT        
  ...
  USE NOAHMP_PARAMETERS, ONLY: ZBOT             
  ...
  USE NOAHMP_PARAMETERS, ONLY: BEXP, PSISAT, SMCMAX, &     
			       GRAV, TFRZ, HFUS            
...
1002      continue
  ...
  USE NOAHMP_PARAMETERS, ONLY: CH2OP, &                                           
			       TFRZ, DENH2O, DENICE, CICE, CWAT, HVAP, HSUB, HFUS 
  ...
  USE NOAHMP_PARAMETERS, ONLY: TFRZ                                     
  ...
  USE NOAHMP_PARAMETERS, ONLY: TFRZ, CICE, CWAT, HFUS                 
  ...
  USE NOAHMP_PARAMETERS, ONLY: TFRZ, DENH2O, DENICE      
  ...
  USE NOAHMP_PARAMETERS, ONLY: SSI, &                        
			       DENH2O, DENICE                
  ...
  USE NOAHMP_PARAMETERS, ONLY: SMCMAX, &                  
                               TIMEAN, FSATMX, &          
                               ISURBAN                    
  ...
  USE NOAHMP_PARAMETERS, ONLY: SMCMAX, KDT, FRZX, SMCWLT, &  
                               ISURBAN                       
  ...
  USE NOAHMP_PARAMETERS, ONLY: SLOPE          
  ...
  USE NOAHMP_PARAMETERS, ONLY: BEXP, DKSAT, DWSAT, SMCMAX     
...
END  SUBROUTINE SHALLOWWATERTABLE
  ...
  SUBROUTINE CARBON (NSNOW  ,NSOIL  ,VEGTYP ,DT     ,ZSOIL  , & 
                     DZSNSO ,STC    ,SMC    ,TV     ,TG     ,PSN    , & 
                     FOLN   ,BTRAN  ,APAR   ,FVEG   ,IGS    , & 
                     TROOT  ,IST    ,LAT    ,ILOC   ,JLOC   , & 
                     LFMASS ,RTMASS ,STMASS ,WOOD   ,STBLCP ,FASTCP , & 
  ...
  USE NOAHMP_PARAMETERS, ONLY: SMCMAX, SLA, NROOT, &             
                               ISBARREN, ISICE, ISWATER, ISURBAN 
  ...
  USE NOAHMP_PARAMETERS, ONLY: FOLNMX, ARM, RMF25, RMR25, RMS25, WDPOOL, WRRAT, LTOVRC,   &
                               TDLEF, DILEFW, DILEFC, FRAGR, TMIN, MRP, &  
			       EBLFOREST                                   
  ...
  real :: r,x
    ...
    STMSMN  = xsamin/sapm
     ...
     TF      = ARM**( (TV-298.16)/10. )
     RESP    = RMF25 * TF * FNF * XLAI * RF * (1.-WSTRES) 
     RSLEAF  = MIN((LFMASS-LFMSMN)/DT,RESP*12.e-6)                         
     
     RSROOT  = RMR25*(RTMASS*1E-3)*TF *RF* 12.e-6         
     RSSTEM  = RMS25*((STMASS-STMSMN)*1E-3)*TF *RF* 12.e-6         
