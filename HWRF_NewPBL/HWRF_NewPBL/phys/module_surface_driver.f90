...
'Lacking arguments for NOAHMPLSM in surface driver')
     ...
     CASE (RUCLSMSCHEME)
       IF (PRESENT(qv_curr)    .AND.  PRESENT(qc_curr) .AND.    &
          ...
          CALL SFCDIAGS_RUCLSM(HFX,QFX,TSK,QSFC,CHS,CHS2,CQS2,T2,TH2,Q2,      &
                     T_PHY,QV_CURR,RHO,P_PHY,                            &
                     CP,R_d,RCP,                                         &
                     ids,ide, jds,jde, kds,kde,                          &
                     ims,ime, jms,jme, kms,kme,                          &
                     i_start(ij),i_end(ij), j_start(ij),j_end(ij), kts,kte    )
     ...
     CASE (PXLSMSCHEME)
       IF (PRESENT(qv_curr)    .AND.  PRESENT(qc_curr) .AND.    &
          ...
          CALL PXLSM(u_phy, v_phy, dz8w, qv_curr, t_phy, th_phy, rho,&
                     ...
                     snoalb, smois, tslb, mavail,T2, Q2,             &
                     zs, dzs, psih,                                  &
...
'Lacking arguments for P-X LSM in surface driver')
     ...
     IF (MYJ) call wrf_error_fatal3("<stdin>",2859,&
