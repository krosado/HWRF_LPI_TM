 ...
 174      grid%em(JA)=EMJ(J)
  ...
  969   continue
    ...
    if(internal_time_loop.eq.1) then      
     ...
     CALL EARTH_LATLON_hwrf (  NHLAT,NHLON,NVLAT,NVLON,  &    
          ...
          CALL G2T2H_hwrf ( SM_G,HRES_SM,   & 
                  NHLAT,NHLON,                 & 
                  grid%DLMD,grid%DPHD,WBD,SBD,           & 
                  tph0d,tlm0d,                 & 
               IDE,JDE,IDS,IDE,JDS,JDE,     & 
                  1,NIDE,1,NJDE,1,1,           &
                  1,NIDE,1,NJDE,1,1,           &
                  1,NIDE,1,NJDE,1,1            )
...
INTEGER, INTENT(IN):: ids,ide,jds,jde,kds,kde, &
 ...
 WRITE(message,*)'------------------------------------------------'
   ...
   do l = 1,num_soil_bot_cat
    ...
    landusef_gc=landusef_out       
    soilcbot_gc=soilcbot_out       
    soilctop_gc=soilctop_out       
  ...
  CALL wrf_dm_bcast_bytes (glat0,4)
  CALL wrf_dm_bcast_bytes (glon0,4)
 ...
 v_out=v_gc
  ...
  IF(BETA1 .NE. 0.)THEN   
    ...
    do l=start_z,end_z
     do j=jts,min(jte,jde-1)
      do i=its,min(ite,ide-1)
         pert_ght(i,j,l) =  (ght_gc(i,j,l) - ght_out(i,j,l))      
    ...
    do l=start_z,end_z
     do j=jts,min(jte,jde-1)
      do i=its,min(ite,ide-1)
         ght_gc(i,j,l) = ght_out(i,j,l) +  beta1*pert_ght(i,j,l)     
  ...
  ELSE 
   ...
   IF(internal_time_loop .le. 2)THEN  
 ...
 END SUBROUTINE hbfilter
  ...
  4   format(f8.2)
      ...
      enddo
       do i=1,jde
        do j=1,ide
      betkc(j,i)=(rgas/grv)*log(0.01*psc(j,i)/pp1(km))  
...
SUBROUTINE EARTH_LATLON_hwrf ( HLAT,HLON,VLAT,VLON,     & 
...
END SUBROUTINE EARTH_LATLON_hwrf
  ...
  SUBROUTINE G2T2H_hwrf( SM,HRES_SM,                          & 
                    HLAT,HLON,                           & 
                    DLMD1,DPHD1,WBD1,SBD1,               & 
                    CENTRAL_LAT,CENTRAL_LON,             & 
                    P_IDE,P_JDE,P_IMS,P_IME,P_JMS,P_JME, & 
                    IDS,IDE,JDS,JDE,KDS,KDE,             & 
                    IMS,IME,JMS,JME,KMS,KME,             &
                    ITS,ITE,JTS,JTE,KTS,KTE              )
      ...
      DO I = ITS,MIN(ITE,IDE) 
       AMAXVAL=0.
          ...
          DO N=1,4
             ...
             ENDIF
             SUM=SUM+NBWGT(N,I,J)
             IF(SUM .GT. 1.0)CALL wrf_error_fatal3("<stdin>",6684,&
