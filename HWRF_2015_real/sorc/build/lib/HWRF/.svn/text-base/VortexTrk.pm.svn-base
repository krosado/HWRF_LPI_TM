...
# MA 02110-1301, USA.
#
# $Id: VortexTrk.pm 115 2011-04-01 15:31:39Z tpbrown@ucar.edu $
#

package HWRF::VortexTrk;
...
HWRF::VortexTrk - Perl extensions for the HWRF Testing and Evaluation suite.

=head1 SYNOPSIS

  use HWRF::VortexTrk;
  my $vtrk = HWRF::VortexTrk->new();
  $vtrk->source();
  $vtrk->compile();

=head1 DESCRIPTION

The B<HWRF::VortexTrk> module provides Perl functions that are commonly used in
...
Instantiates an HWRF::VortexTrk object.
...
sub new {
	...
	if (! defined $self->{DST}) {
		if (defined $self->{TOPLEVEL}) {
			...
			$self->{DST} = abs_path("gfdl-vortextracker");
		}
	}
	...
	if (! defined $self->{SRC}->{PKG}) {
		$self->{SRC}->{PKG} = "GFDL-VORTEXTRACKER.tar.gz";
	}
	if (! defined $self->{CONFIG}->{PAR}) {
		$self->{CONFIG}->{PAR} = "serial";
	}
	if (! defined $self->{CONFIG}->{FILE}) {
		$self->{CONFIG}->{FILE} = "configure.trk";
	}
	if (! defined $self->{EXES}) {
		$self->{EXES} = [
			"trk_exec/hwrf_gettrk.exe",
                        "trk_exec/hwrf_vint.exe",
                        "trk_exec/hwrf_tave.exe",
		];
	}
...
sub envs {
	...
	my (%args) = @_;
        ...
        my $out={ LIB_G2_PATH => $ENV{'LIB_G2_PATH'} };
        foreach my $var (qw{ LIB_Z_PATH LIB_JASPER_PATH LIB_PNG_PATH }) {
            if(defined($ENV{$var})) {
                $out->{$var}=$ENV{$var};
            } else {
                $out->{$var}='/usr/lib64';
