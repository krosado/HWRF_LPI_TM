...
!
! main program: gdm3prep
!   PRGMMR: RICHARD YABLONSKY, URI/GSO, 2014-04-05
!   LANG: FORTRAN 90
! Abstract: Read eastpac unsharpened gdem3 data (NetCDF) and prepare
...
IMPLICIT NONE
 ...
 REAL, DIMENSION(nl)                :: zgdem3, taa, saa, area
 ...
 REAL, DIMENSION(nx,ny)             :: yf
 REAL, DIMENSION(ih,jh)             :: dpth, lmsk
...
! MPIPOM-TC subset of 1/4-deg gdem3 grid for selected region
 ...
 IF (odata == 'g3tr') THEN
   ...
   blon =  -98.5
   blat =   10.0
 ELSE IF (odata == 'g3ep') THEN
   ncxstart = (/770/)
   ncystart = (/349/)
   nc3start = (/770, 349, 1/)
   blon = -167.5
   blat =    5.0
 ELSE IF (odata == 'g3wp') THEN
   ncxstart = (/387/)
   ncystart = (/349/)
   nc3start = (/387, 349, 1/)
   blon =   96.6
   blat =    5.0
 ELSE IF (odata == 'g3ni') THEN
   ncxstart = (/130/)
   ncystart = (/339/)
   nc3start = (/130, 339, 1/)
   blon =   32.3
   blat =    2.5
 ELSE IF (odata == 'g3si') THEN
   ncxstart = (/130/)
   ncystart = (/169/)
   nc3start = (/130, 169, 1/)
   blon =   32.3
   blat =  -40.0
 ELSE IF (odata == 'g3sw') THEN
   ncxstart = (/387/)
   ncystart = (/169/)
   nc3start = (/387, 169, 1/)
   blon =   96.6
   blat =  -40.0
 ELSE IF (odata == 'g3se') THEN
   ncxstart = (/721/)
   ncystart = (/169/)
   nc3start = (/721, 169, 1/)
   blon = -179.8
   blat =  -40.0
 ELSE IF (odata == 'g3sa') THEN
   ncxstart = (/1106/)
   ncystart = (/169/)
   nc3start = (/1106, 169, 1/)
   blon =  -83.5
...
! Read unsharpened t and s gdem3 data (NetCDF)
 ...
 CALL check(NF90_GET_VAR(fid,idepid,zgdem3,nczstart,nczcount))
...
! Interpolate t and s onto 1/12-deg MPIPOM-TC grid
 ...
 END DO
  ...
  IF (odata == 'g3tr' .OR. odata == 'g3ep' .OR. &
      odata == 'g3se' .OR. odata == 'g3sa') THEN
 DO i = 1,nx
   x0(i) = x0(i)-360.
 END DO
  END IF
 ...
 DO k = 1,nl
   DO j = 1,ny
     ...
     END DO
   END DO
   call horinterp(-32000.,im,jm,nx,ny,x0,y0,xi,yi,yf,yfi)
   DO j = 1,jm
     DO i = 1,im
       t(i,j,k) = yfi(i,j)
     END DO
...
! MPIPOM-TC bottom topography based on gdem3
...
! Remove gdem3-based bottom topography via neutral profile generation
...
! Read and assimilate gfs sst into temperature field
 ...
 224 format(1x,2i5)       ! ensure mask.gfs.dat is linked to fort.22
     ...
     call mixsstz(t,stm,h,fsm,zgdem3,im,jm,nl,0)
...
! Create MPIPOM-TC input ts_initial file
 ...
 CALL check(NF90_ENDDEF(fid))
 CALL check(NF90_PUT_VAR(fid,z_varid,zgdem3))
