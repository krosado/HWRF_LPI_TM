...
c 2010-10-18  Sam Trahan  Changed the "no more tracks" hour from 99 to 999
c                       to allow for tracking at hour 99.
c
c Input files:
...
c                the input model (999 = no more times available).
...
c
      ...
      real      gridpoint_maxmin,xinp_fixlat,xinp_fixlon
...
c     First, allocate the working data arrays....
     ...
     &    iva /= 0 .or. isa /= 0) then
      ...
      ifh = 1
      ifhloop: do while (ifhours(ifh) /= 999 .and. 
     &                   ifh <= maxtime)
...
c             flag will have a value of 'U', for "undetermined".
     ...
     &                          ,clon(ist,ifh,1),clat(ist,ifh,1),dist)
                  if (dist > max_mslp_850) then
                    ...
                    print *,' '
                    print *,'Distance between the parm centers for 850'
                    print *,'zeta and mslp is ',dist,' (km)'
                  endif
                endif
...
c             knots (1.9427) is explained in output_atcf.
     ...
     &                        ,gridprs(ist,ifh),vradius,ioaxret)
                  endif
                else
...
c      parameter (jf=750*750,lugb=11,lugi=31) 
      parameter (jf=16000000,lugb=11,lugi=31) 
      real      f(jf)
...
c
      ...
      print *,'Values of 999 mean undefined: '
      write (6,87) itmphrs
...
c      parameter (lugb=11,lugi=31,jf=750*750)
      parameter (lugb=11,lugi=31,jf=16000000)
