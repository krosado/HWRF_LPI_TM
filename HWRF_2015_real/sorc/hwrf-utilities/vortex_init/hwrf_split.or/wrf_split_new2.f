...
! read data
      ...
      READ(5,*)ITIM,IBGS,IVOBS,basin,iflag_cold
...
! constant P
      DO K=1,NZ+1
        P2(2*K-1)=P1(IPX,JPX,K)
        SIG(2*K-1)=P1(IPX,JPX,K)/PSFX
      END DO
      DO K=1,NZ
!        P2(2*K)=EXP((ALOG(P1(IPX,JPX,K))+ALOG(P1(IPX,JPX,K+1)))*0.5)
        P2(2*K)=PM1(IPX,JPX,K)
        SIG(2*K)=PM1(IPX,JPX,K)/PSFX
      END DO
...
!       DO KST=1,KSTM
     ...
     &  KMX,1,NX,1,NY,IBGS,IVOBS,basin,iflag_cold,I360)
...
!CC  1 FORMAT(/'...FMT=',A10,/,' ...BUFF=',A100)
   ...
   32 FORMAT(A130)
     ...
     &     jswin,jewin,IBGS,IVOBS,basin,iflag_cold,I360)
!
! ITIM: Time level
...
!      END IF 
      ...
      IF((R.LT.R0(IC)).and.R.GT.(R0(IC)-0.4))THEN
        IB5=IB5+1
        ING5(IB5)=I
        JNG5(IB5)=J
        WT2=min(1.0,(R0(IC)-R)/0.4)
        RRIJ(IB5)=WT2*WT2*(3.-2.*WT2)
      END IF
