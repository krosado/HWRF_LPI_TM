...
class WPSTask(HWRFTask):
    ...
    def __init__(self, dstore, conf, section, sim, domains, taskname=None, 
        ...
        """Create a new WPSTask for the specified Datastore dstore and
        given section of the HWRFConfig conf.  The "sim" is the
        WRFSimulation for which we are preparing input.  The domains
        must be a list of WRFDomain objects.  Optional arguments:

          taskname - the name of this Task.  Default: config section name.

          wpsdir - the directory in which to run WPS.  Default: taskname
             subdirectory of conf.getdir("WORKhwrf")

          starttime, endtime - simulation length.  Default: same as sim.
          increment - boundary input interval.  Default: get from sim.
          
          parent_atime - analysis time of parent model.  Default: starttime
             
          geogrid_from - a Geogrid object whose output should be
             linked before running this WPS step.  This is used when
             the WPS Ungrib and Metgrid are run more than once on the
             same domain.  For example, one may run multiple analysis
             times or multiple forecast lengths off of the same
             geogrid output.
          
        Other options are sent to the HWRFTask constructor.
        """
    ...
    def products(self,time=None,domain=None):
        """Iterates over all products
          domain - the WRFDomain
          time - the time
        One, both or neither can be specified.  All matching
    ...
    def deliver_products(self,time=None,domain=None,fromloc=None,
        ...
        intercom directory.  The optional time and domain arguments
        are sent to self.products.  It assumes that the file to be
        delivered is in the current working directory with the same
        name as the destination file.  To change that, give a lambda
        function in "fromloc", which converts the destination filename
        (the only argument) to a local filename.  The "keep" argument
        has the same meaning as in deliver_file: if False, the file
        may be moved to the destination.  If "relink" is true, and the
        file is moved, then a symlink will be made from the original
        file location to the destination."""
        logger=self.log()
        if time is None:
            time=self.starttime
        bad=False
...
class Geogrid(WPSTask):
    ...
    def __init__(self,*args,**kwargs):
        """Creates a new Geogrid.  All arguments are passed to the
        WPSTask constructor."""
    ...
    def make_products(self):
        ...
        Implementation note: If you change the list of products, make
        sure all geogrid outputs needed as input to ungrib or metgrid
        have a prodname that is the same as the """
...
class Ungrib(WPSTask):
    """This is a HWRF task that extracts the meteorological fields from
    GRIB formatted files and write the fields to intermediate files."""
    ...
    def input_at(self,dt,igrib=1,require=False):
        ...
        return ret
    def get_grib(self,require=False):
...
class Metgrid(WPSTask):
    ...
    def __init__(self, *args,**kwargs):
        """Creates a new Metgrid.  All arguments are passed to the
        WPSTask constructor."""
        super(Metgrid, self).__init__(*args,**kwargs)
        self._n_gribfiles=0
    def run(self):
        """Copies inputs, runs metgrid, delivers outputs.  Note that
        ungrib must have been run in the same directory beforehand.
        The geogrid must have been run in the same directory, unless
        it was passed as input via geogrid_from= in the constructor."""
        logger=self.log()
        try:
            produtil.fileop.makedirs(self.outdir)
