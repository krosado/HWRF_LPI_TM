...
class NHCProducts(hwrf.hwrftask.HWRFTask):
    ...
    def canrun(self,silent=True):
        ...
        If silent=True, does not log anything."""
    ...
    def write_namelist(self,f,wrf,moad,inner):
        ...
        directly.  Writes the products.nml namelist to file object
        f."""
        ...
        repl={ 'dx':g('domains','dx'),
               ...
               'inhifreq':os.path.join(str(wrftask.location),inner.
                                       hifreq_file()),
    ...
    def run(self,deliver_wrfdiag=False):
        ...
        with TempDir(prefix=prefix,dir=dir,logger=logger):
            ...
            make_symlink(os.path.join(self._wrftask.location,'MDstatus'),
                         'MDstatus')
            checkrun(openmp(exe(runme),threads=self.confint(
                        'threads',int(os.environ.get('NHC_PRODUCTS_NTHREADS',
                                                     '1')))),logger=logger)
