...
def disable_locking():
    """Entirely disables all locking in this module.  If this is
    called, any locking attempts will raise LockingDisabled.  That
    exception derives directly from BaseException, which well-written
    Python code will never catch, hence ensuring a rapid, abnormal
    exit of the program.  This routine should never be called
    directly: it is only used as part of the implementation of the
    produtil.sigsafety, to prevent file locking after catching a
    terminal signal, hence allowing the program to exit as quickly as
    possible, and present a stack trace to any locations that attempt
    locking."""
    global locks_okay
    locks_okay=False
    for lock in locks:
...
class LockingDisabled(BaseException):
    ...
    This is a subclass of BaseException, not Exception, to attempt to
    cleanly kill the thread."""
...
class LockFile(object):
    ...
    def __init__(self,filename,until=None,logger=None,max_tries=10,sleep_time=3,first_warn=0,giveup_quiet=False):
        ...
        Optional arguments:
          ...
          max_tries -- maximum tries before giving up on locking
          sleep_time -- approximate sleep time between locking attempts.  
          first_warn -- first locking failure at which to write warnings 
            to the logger
          giveup_quiet -- do not log the final failure to lock"""
        if not locks_okay:
