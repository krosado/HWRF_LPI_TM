...
logthread=''  # string for log messages to indicate thread number/name 
jlogdomain='jlog'
...
class MasterLogFormatter(logging.Formatter): 
    ...
    def format(self, record):
        ...
        # Formatter.format: It ignores formatException (never calls
        # it) and caches the exception info, even if the formatter is
        # not supposed to output it.
        global logthread
...
message, the log file is opened, the message is written and the file
is closed.  This is done to mimic the postmsg command.  Exception
information is never sent to the log file."""
...
class JLogHandler(MasterLogHandler):
    """This is a custom loging Handler class for the jlogfile.  It has a
higher minimum log level for messages not sent to the jlogfile domain.
Also, for every log message, the log file is opened, the message is written
and the file is closed.  This is done to mimic the postmsg command.  
...
def mpi_redirect(threadname,stderrfile,stdoutfile,
    ...
    """When the Python script splits itself into multiple processes
    via MPI, this function is called to redirect stdout to stdoutfile,
    ...
    if stderrfile is not None:
        ...
        # file as well.
        mloghandler=MasterLogHandler(olderr,jlogdomain,masterlevel,oformat,
                                     oformat)
        logger.warning('Turning on logging of high priority messages to '
                       'original stderr stream.')
        if masterlevel!=logging.NOTSET: mloghandler.setLevel(masterlevel)
...
def configureLogging(jlogfile=None,
    ...
    Options:
        jlogfile = path to the jlogfile. Default: use
            os.environ('jlogfile') if set.  Otherwise, stderr.
        level = minimum logging level globally.  Set to INFO by default.
            Change this to logging.DEBUG if you're debugging the program.
        jloglevel = minimum logging level to send to jlogfile
        japplevel = minimum logging level to send to jlogfile from all
        ...
        eloglevel = minimum logging level to send to stderr from ALL logs
            Set to None to disable stderr logging
        ologlevel = minimum logging level to send to stdout from ALL logs
