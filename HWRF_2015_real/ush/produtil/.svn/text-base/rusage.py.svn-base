...
def setrlimit(logger=None, ignore=False, hard=False, **kwargs):
    """Sets resource limits.  If ignore=True, ignores any errors from
    getrlimit or setrlimit.  If hard=True, attempts to set hard
    limits, which generally requires administrator privilages.  The
    logger argument sets the logger (default: produtil.setrlimit
    logging domain).  The kwargs should be a list of resource limits
    from this list:
...
def getrlimit(logger=None):
    """Gets the current resource limits.  If logger is not None, sends
    the limits to the logger at level INFO."""
...
class RUsage(object):
    ...
    def __init__(self,who=resource.RUSAGE_CHILDREN,logger=None):
        ...
        Pass who=resource.RUSAGE_SELF to get usage on this process or
        rusage.RUSAGE_CHILDREN (the default) to get resource usage on
        child processes."""
        self.logger=logger
    ...
    def who(self):
        ...
        (RUSAGE_CHILDREN) or this process (RUSAGE_SELF) .  See the
