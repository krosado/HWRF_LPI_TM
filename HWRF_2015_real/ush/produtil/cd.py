...
class TempDir(object):
    ...
    def __init__(self,suffix='.tmp',prefix='tempdir.',dir=None,keep=False,
        ...
        """Creates a TempDir:
          ...
          add_perms - permissions to add to the directory.  Default: 755.

          remove_perms - permissions to remove from the directory.
            Default: setuid and world write.

          cd -- if True (default), cd to the directory in the "with"
    ...
    def name_make_dir(self):
        ...
        os.chmod(self.dirname, (s.st_mode | int(self._add_perms)) 
                 & ~self._remove_perms )
    def mkdir_cd(self):
        """This subroutine creates the temporary directory and chdirs
        the current process into that directory.  It calls
        self.name_make_dir() to do the naming and directory
        creation."""
    ...
    def cd_out(self):
        ...
        mkdir_cd and returns to the original directory, if
        possible."""
        if self._logger is not None:
    ...
    def cd_rmdir(self):
        ...
        It is the caller's responsiblity to ensure this function is
    ...
    def __exit__(self,etype,value,traceback):
        """This is a simple wrapper around cd_rmdir that is intended
        to be used with in a "with" block.  This subroutine is
...
class NamedDir(TempDir):
    ...
    def __init__(self,dirname,keep=True,logger=None,keep_on_error=True,
        ...
        vs. non-deletion options:
           ...
           keep - if False, the file is deleted upon successful return
             of the "with" block.  If True, the file is kept upon
             successful return.

           keep_on_error - controls deletion upon catching of an
             Exception or GeneratorException (or subclass thereof).

           add_perms - permissions to add to the directory after cding
             into it.  Default: none.

           remove_perms - permissions to remove from the directory after
             cding into it.  Default: none.

           rm_first - if the directory already exists, delete it first
