"""This module is a workaround for a defficiency of Python.  When
Python receives a fatal signal other than SIGINT, it exits immediately
...
The base class of any exeption thrown due to a signal is CaughtSignal.
...
def install_handlers(ignore_hup=False,raise_signals=[signal.SIGTERM,signal.SIGINT,signal.SIGQUIT]):
    """Installs signal handlers that will raise exceptions.

        ignore_hup - If True, SIGHUP is ignored, else SIGHUP will
            raise HangupSignal

        raise_signals - List of exceptions that will raise
