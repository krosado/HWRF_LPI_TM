...
class FakeException(Exception):
    """This is used to get a stack trace."""
...
class Datastore(object):
    """Stores information about Datum objects in a database.  Each
    object has a type, a string location, an integer "available"
    parameter, and arbitrary (key,value) metadata pairs.  This object
    can safely be accessed by multiple threads in the local process,
    and handles concurrency between processes via file locking."""
    def __init__(self,filename,logger=None,locking=True):
        """Creates a Datastore for the specified sqlite3 file.  Uses
        the given logging.Logger object for logging messages.  Set
    ...
    def _createdb(self,con):
        """Creates the tables used by this Datastore.  This code must
        be executed inside a transaction() and should only be executed
        on initial opening of the file, in the Datastore constructor."""
...
class Transaction(object):
    """Implements sqlite3 transactions on a Datastore.  A transaction
    is a set of reads and updates that should either ALL be done, or
    NONE at all.  Transactions also speed up the script, sometimes by
    as much of a factor of 300, by grouping I/O operations to the
    ...
    def init_datum(self,d,meta=True):
        ...
        there already and fill the object with data from the
        database."""
    ...
    def update_datum(self,d):
        """Given a Datum whose location and availability is set,
        update that information in the database, adding the Datum if
        necessary."""
    ...
    def set_meta(self,d,k,v):
        """Sets metadata key k to value v for the given Datum.  If k
        is location or available, then the product table will be
        updated instead."""
        if k=='location':
...
class Datum(object):
    ...
    def getprodtype(self):
        """Returns the product type of this Datum.  This is generally
        the name of the Python class that created the entry in the
        database."""
    ...
    category=property(getcategory,None,None,"""the caregory (read-only)""")
    ...
    def __hash__(self): return hash(self._prodname)^hash(self._category)
    def __str__(self): return '%s with id %s'%(self.prodtype,self.did)
    ...
    def __contains__(self,k):
        """Returns True if the specified metadata key is set, and
        False otherwise.  Always returns True for 'available' and
        'location'."""
...
class CallbackExceptions(Exception):
    """This exception is raised when a Product class encounters
...
class Product(Datum):
    """A Product is a piece of data that can be produced by a Task.
    Once the product is available, self.available or
    self.is_available() will be True, and the self.location will be
    valid.  The meaning of self.location is up to the implementer to
    decide, but it should be a full path to a location on disk for
    file products.  As with all Datum objects, a Product also has
    arbitrary metadata."""
    def add_callback(self,callback,args=None,states=None):
        """Adds a delivery callback function.  This is a function to
        be called when the product is delivered to do such tasks as
        running an NCO dbn_alert, or copying to a website, or emailing
        someone.  This function is only added in this local Python
        instance, not in the database file.  Also, it is the
        responsibility of the subclasses to call self.call_callbacks()
        from self.deliver() to ensure the callbacks are run."""
        if args is None: 
            largs=list()
        else:
            largs=list(args)
    ...
    def call_callbacks(self,logger=None):
        """Calls all data delivery callbacks for this Product.
        Collects any raised Exception subclasses until after all
        callbacks are called.  Will raise CallbackExceptions if any
    ...
    def check(self,**kwargs):
        """Checks to see if this product is available.  This is
        generally not a cheap operation, as it can take seconds or
        minutes and may fail.  One should call "available" instead if
        cached information is sufficient."""
        self.update()
    ...
    def undeliver(self,**kwargs):
        """"Undelivers" a product.  The meaning of that is
        implementation-dependent: it could mean deleting an output
        file, or any number of other actions.  Regardless, it should
        result in self.available=False or an exception being thrown.
        Note that this is generally an expensive operation that could
        take seconds or minutes, and may fail.  The default
        implementation simply sets available to False."""
        self.available=False
    def setavailable(self,val):
        """Sets the "available" attribute of this Product in the
        database after converting the given value to a bool and then
        int (int(bool(val)))."""
    ...
    def validate(self):
        """Validates this object's Datastore, prodname and category.
        Adds the requirement that the category must not contain double
        stars (**)."""
...
class UpstreamFile(Product):
    """This subclass of Product represents a file that is created by
    an external workflow.  It implements a check() call that
    determines if the file is larger than a minimum size and older
    than a minimum age.  Once the file is large enough and old enough,
    it sets the location and availability.  Any calls to undeliver()
    ...
    def check(self,frominfo=None,minsize=None,minage=None,logger=None):
        ...
        Updates the "available" and "location" attributes of this
        Product.  Uses two metadata values to decide whether the file
        is "available" if it exists:
...
def wait_for_products(plist,logger,renamer=None,action=None,
    ...
    plist - a Product or a list of Product objects.

    logger - the logging.Logger object in which to log messages.

    renamer - a function or callable object that provides a new name
       for each product.  This is passed the product, the logger and
       the contents of *renamer_args.  
       Default: os.path.basename(p.location)

    action - an action to perform on each product.  This is passed the
       product, the output of renamer, the logger and the contents of
       *action_args.
       Default: perform no action.

    renamer_args - Optional: arguments to renamer.  See above.

    action_args - Optional: arguments to action.  See above.

    sleeptime - after checking availability of all products, if at
       least one is unavailable, the code will sleep for this much
       time before rechecking.  Will be overridden by 0.01 if it is
       set to something lower than that.
       Default: 20

    maxtime - maximum amount of time to spend in this routine before
       giving up.
    """
    if renamer is None:
...
class Task(Datum):
    ...
    @property
    def jlogfile(self):
        """Returns the jlogfile logger.  The jlogfile is intended to
        receive only major errors, and per-job start and completion
        information."""
        return produtil.log.jlogger

    def postmsg(self,message,*args,**kwargs):
        """Sends a message to the logger at level INFO.  You can use
        self.jlogfile to send other levels."""
        produtil.log.jlogger.info(message,*args,**kwargs)

    def setstate(self,val):
        """Sets the "available" attribute to the specified integer.
        For compatibility with other scripts, this should be FAILED,
        UNSTARTED, RUNNING, PARTIAL or COMPLETED."""
        self['available']=int(val)
    def getstate(self): 
        """Returns the "available" attribute as an integer.  This is
        used as the state of the Task.  Typically, the return value
        should be one of: FAILED, UNSTARTED, RUNNING, PARTIAL, or
        COMPLETED."""
    ...
    def products(self,*args,**kwargs):
        ...
        products.  All subclasses should reimplement this method, and
    ...
    def log(self):
        ...
        return self._logger
    def clean(self):
        """Cleans up any unneeded data used by this task.  This
    ...
    def completed(self):
        ...
        return self.state==COMPLETED
    def runpart(self):
        """Performs a subset of the work that this task is supposed to
        do and returns.  This is intended to be used for tasks that
        can be broken up into small pieces, such as post-processing
        all output files from a NWP simulation one by one.  The
        default implementation simply calls self.run()"""
        self.run()
